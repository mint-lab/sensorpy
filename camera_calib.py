...
def show_reproject_pts(cam_img, cam_K, cam_distort, cam_rvec, cam_tvec, cam_pts, obj_pts, n_rows):
    if n_rows > 0:
        plt.figure()
        for i in range(len(cam_img)):
            ...
            proj, _ = cv.projectPoints(obj_pts[i], cam_rvec[i], cam_tvec[i], cam_K, cam_distort)
...
if __name__ == '__main__':
    ...
    if config['calib_output']:
        with open(config['calib_output'], 'wt') as f:
            calib_result = {'cam_K': cam_K.tolist(), 'cam_distort': cam_distort.tolist()}
    ...
    show_reproject_pts(cam_img, cam_K, cam_distort, cam_rvec, cam_tvec, cam_pts, obj_pts, config['img_plot_rows'])
    plt.show()
